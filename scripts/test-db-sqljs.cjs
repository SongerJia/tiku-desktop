// db.js 集成测试基线 —— SQL.js 驱动变体（验证 P1 存储驱动抽象：同一套断言在 SQL.js 下也通过）
// 运行：npx electron scripts/test-db-sqljs.cjs
// 说明：db.js 依赖 electron（db-assets nativeImage），仍用 electron 主进程跑；SQL.js 是 wasm 同步 API。
...
function ok(name, cond) {
  if (cond) { pass++; require("fs").appendFileSync("sqljs-test-out.txt", "OK " + name + "\n") } else { fail++; require("fs").appendFileSync("sqljs-test-out.txt", "FAIL " + name + "\n") }
}
...
try {
  ...
  // 与 db.js dbPath() 保持一致（userData/tiku.db），autoBackup/restoreBackup 按此路径操作物理文件
  const sqljsFile = path.join(tmp, 'tiku.db')
  await db.initAsync(await createSqlJsDriver({ file: sqljsFile }))
  ...
  // 统一走驱动 API（db._raw 是驱动对象，prepare/run 双驱动签名一致）
  const catT = db.getCategories()[0]
  db._raw.prepare('UPDATE categories SET client_id=NULL WHERE id=?').run(catT.id)
  ...
  require("fs").appendFileSync("sqljs-test-out.txt", `db 集成测试：${pass} 通过 / ${fail} 失败\n`)
  try { app.exit(fail ? 1 : 0) } catch (e) { process.exit(fail ? 1 : 0) }
  }).catch(e => {
    fail++
    console.error('❌ db 集成测试异常:', e && e.stack ? e.stack.split('\n').slice(0, 4).join('\n') : e)
    require("fs").appendFileSync("sqljs-error.txt", "EXC " + (e && e.stack ? e.stack.split("\n").slice(0,6).join(" | ") : e) + "\n")
    try { app.exit(fail ? 1 : 0) } catch (e2) { process.exit(fail ? 1 : 0) }
  })
} catch (e) {
  fail++
  console.error('❌ db 集成测试异常:', e && e.stack ? e.stack.split('\n').slice(0, 4).join('\n') : e)
