...
/* ② 问候卡装饰波纹（右上同心圆环，overflow hidden 裁切；缓慢波纹呼吸） */
.greet-card::after {
  ...
  pointer-events: none;
  animation: greetRing 4.5s ease-in-out infinite;
}
@keyframes greetRing {
  0%, 100% { box-shadow: 0 0 0 14px color-mix(in srgb, var(--brand) 6%, transparent), 0 0 0 30px color-mix(in srgb, var(--brand) 4%, transparent); opacity: .8; }
  50% { box-shadow: 0 0 0 24px color-mix(in srgb, var(--brand) 8%, transparent), 0 0 0 52px color-mix(in srgb, var(--brand) 5%, transparent); opacity: 1; }
}
